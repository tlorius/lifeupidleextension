...
export function castCombatSpell(
  ...
  const restoreMana = (rawMana: number): number => {
    const manaRestore = Math.max(1, Math.round(rawMana));
    nextState = {
      ...nextState,
      resources: {
        ...nextState.resources,
  ...
  if (spellId === "arcane_bolt") {
    ...
    const totalStats = getTotalStats(nextState);
    const volleyCount = 3 + getActiveClassNodeRank(nextState, "archer_2");
    ...
    const tickCount = 3 + Math.floor(ignitionRank / 2);
    const tickDamage =
    ...
    const convertedDamage =
      ...
      idleSeconds * (0.02 + streakRank * 0.004);
    ...
    const cooldownReductionMs = 6000 + momentumRank * 500;
    ...
    const petHit =
      ...
      (totalStats.petStrength ?? 0) * (14 + bondRank * 2);
